...
const adminService = {
  ...
  addBed: (roomId, bedNumber) =>
    api.post(`/admin/beds/room/${roomId}`, { bedNumber }),
  ...
  // NEW: Add these for the approval flow
  approveResident: (id) => api.put(`/admin/residents/approve/${id}`),
  rejectResident: (id) => api.put(`/admin/residents/reject/${id}`),
  getComplaints: (hostelId, status) =>
    api.get(`/admin/complaints/hostel/${hostelId}?status=${status || ""}`),
  resolveComplaint: (id) => api.put(`/admin/complaints/resolve/${id}`),

  getShiftRequests: (hostelId, status) =>
    api.get(
      `/admin/room-shift/hostel/${hostelId}?status=${status || "PENDING"}`,
    ),
  ...
  addNotice: (hostelId, data) =>
    api.post(`/admin/notices/hostel/${hostelId}`, data),

  getMessMenu: (hostelId) => api.get(`/admin/mess/hostel/${hostelId}`),
  addMessItem: (hostelId, data) =>
    api.post(`/admin/mess/hostel/${hostelId}`, data),
};
